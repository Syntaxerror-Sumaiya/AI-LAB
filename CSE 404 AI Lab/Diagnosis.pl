...
symptom(Patient,allergies) :-
    write_list(['Does ',Patient,' have a allergies (y/n) ?']),
    response(Reply),
    Reply='y'.


symptom(Patient,diarrhea) :-
    write_list(['Does ',Patient,' have a diarrhea (y/n) ?']),
    response(Reply),
    Reply='y'.


symptom(Patient,mononucleosis) :-
    write_list(['Does ',Patient,' have a mononucleosis (y/n) ?']),
    response(Reply),
    Reply='y'.

symptom(Patient,irritation) :-
    write_list(['Does ',Patient,' have a irritation (y/n) ?']),
    response(Reply),
    Reply='y'.

symptom(Patient,puffy) :-
    write_list(['Does ',Patient,' have a puffy (y/n) ?']),
    response(Reply),
    Reply='y'.

symptom(Patient,inflamed) :-
    write_list(['Does ',Patient,' have a inflamed (y/n) ?']),
    response(Reply),
    Reply='y'.
